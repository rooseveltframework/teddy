describe('Misc', function() {
  it('should render {variables} (misc/variable.html)', function(done) {
    assert.equalIgnoreSpaces(teddy.render(getTemplate('misc/variable.html'), model), '<p>Some content</p>');
    done();
  });

  it('should render multiple {variables} (misc/multipleVariables.html)', function(done) {
    assert.equalIgnoreSpaces(teddy.render(getTemplate('misc/multipleVariables.html'), model), '<p>Some content</p> <h5>More content</h5>');
    done();
  });

  it('should render nested {variables} (misc/nestedVars.html)', function(done) {
    assert.equalIgnoreSpaces(teddy.render(getTemplate('misc/nestedVars.html'), model), '<p>Variable with a variable inside: And another: Some content</p>');
    done();
  });

  it('should properly escape HTML entities present in {variables} (misc/varEscaping.html)', function(done) {
    assert.equalIgnoreSpaces(teddy.render(getTemplate('misc/varEscaping.html'), model), '<p>&lt;span&gt;raw html&lt;/span&gt;</p>');
    done();
  });
  ...
  it('should trigger caching rollover given one template with 100 unique models (misc/variable.html)', function(done) {
    ...
    for (i = 0; i < 100; i++) {
      teddy.render(getTemplate('misc/variable.html'), {something: i});
    }
  ...
  it('should not cache a blacklisted template (misc/variable.html)', function(done) {
    ...
    teddy.render(getTemplate('misc/variable.html'), {something: 1});
  ...
  it('should only cache whitelisted templates (misc/variable.html)', function(done) {
    ...
    teddy.render(getTemplate('misc/plainHTML.html'), {something: 1});
    teddy.render(getTemplate('misc/variable.html'), {something: 1});
  ...
  it('should render a template with missing or invalid model (misc/emptyModelMarkup.html)', function(done) {
    assert.equalIgnoreSpaces(teddy.render(getTemplate('misc/emptyModelMarkup.html'), 1), '<div><p>Hello</p></div>');
    done();
  });

  it('should not render {variables} that don\'t exist in the model (misc/varNotInModel.html)', function(done) {
    assert.equalIgnoreSpaces(teddy.render(getTemplate('misc/varNotInModel.html'), model), '{noExist}');
    done();
  });
  ...
  it('should minify template with internal minifier (misc/templateToMinify.html)', function(done) {
    teddy.minify(true);
    assert.equal(teddy.render(getTemplate('misc/templateToMinify.html'), model), '<!DOCTYPE html><html lang=\'en\'> <head> <meta charset=\'utf-8\'> <meta name=\'viewport\' content=\'width=device-width,initial-scale=1\'> <meta name=\'format-detection\' content=\'telephone=no\'> <title>Plain HTML</title> </head> <body> <main> <p>This template contains no teddy tags. Just HTML.</p> </main> </body></html>');
    teddy.minify(false);
    done();
  });
});
